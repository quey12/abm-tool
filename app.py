...
@app.route('/liquor-stores')
def liquor_stores_dashboard():
    conn = sqlite3.connect('data/abm_tool.db')
    c = conn.cursor()
    c.execute("SELECT DISTINCT i.signal_id, c.name, i.phone, i.address, i.url FROM liquor_stores i JOIN companies c ON i.company_id = c.company_id ORDER BY i.signal_id DESC")
    signals = c.fetchall()
    conn.close()
    print(f"Liquor stores fetched: {len(signals)}")  # Debug
    return render_template('dashboard.html', signals=signals, signal_type='LiquorStores', title='Liquor Stores')
